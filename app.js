...
const App = {
    ...
    updateSelectedDisplay() {
        ...
        this.selectedHeadphones.forEach(id => {
            const headphone = DataManager.getById(id);
            if (!headphone) return;
    ...
    compare() {
        ...
        // 收集数据
        const headphonesData = Array.from(this.selectedHeadphones).map(id => DataManager.getById(id)).filter(Boolean);
